use fracture_ir::{Program, Module, Function, Inst, Value, ModulePath, PathSegment, Visibility, UseTree, ImportType, ModuleData, Shard};
...
pub fn check(program: &Program) -> Result<(), String> {
    if !program.active_glyphs.is_empty() {
        println!("[Resolution] Active glyphs: {:?}", program.active_glyphs);
    }
...
impl<'a> Resolver<'a> {
        ...
        if let ModuleData::Shard(shard) = &module.data {
            ...
            for (child_name, child_shard) in &shard.children {
                ...
                if current_path.segments.len() == 1 && matches!(current_path.segments[0], PathSegment::Shard) {
                    child_path_segments = vec![PathSegment::Shard, PathSegment::Ident(child_name.clone())];
                }
                else {
                    child_path_segments.push(PathSegment::Ident(child_name.clone()));
                }

                let child_path = ModulePath { segments: child_path_segments };

                // TODO: Refactor to avoid this conversion
                ...
                self.check_module(&child_module, &child_path)?;
            }
        }
    ...
    fn resolve_imports(&self, module: &Module, current_mod_path: &ModulePath) -> Result<ImportMap, String> {
        ...
        for use_stmt in &module.uses {
            match use_stmt.import_type {
                ImportType::Shard => {
                    self.expand_use_tree(&use_stmt.tree, current_mod_path, &mut imports)?;
                }
                ImportType::Glyph => {
                    // Skip glyph imports - they're not file-based modules
                }
            }
        }
        ...
        match tree {
            UseTree::Simple { path, alias } => {
                if let Ok(target_func) = self.resolve_absolute_path(path, current_mod_path) {
                    ...
                    if let Err(_) = self.find_shard(&canon) {
            ...
            UseTree::Glob { path } => {
                let canon = self.canonicalize(path, current_mod_path)?;
                let target_shard = self.find_shard(&canon)?;

                for (func_name, func) in &target_shard.functions {
    ...
    fn find_shard(&self, canon_path: &ModulePath) -> Result<&Shard, String> {
        let root_shard = match &self.program.root_module.data {
            ModuleData::Shard(s) => s,
            ModuleData::Glyph(_) => return Err("Root module is not a shard".to_string()),
        };

        let start_idx = if !canon_path.segments.is_empty() && matches!(canon_path.segments[0], PathSegment::Shard) { 1 } else { 0 };

        let mut current_shard = root_shard;

        for segment in &canon_path.segments[start_idx..] {
            match segment {
                PathSegment::Ident(name) => {
                    current_shard = current_shard.children.get(name)
